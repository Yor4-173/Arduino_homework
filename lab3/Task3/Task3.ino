...
void sendMessage(float h, float t, float lux) {
  ...
  char encrypted[128];
  uint16_t encryptedLength = aesLib.encrypt64((byte*)json, strlen(json), encrypted, aes_key, 128, aes_iv);
...
String decryptPayload(String base64Input) {
  ...
  byte decrypted[128];
  uint16_t decryptedLength = aesLib.decrypt64(cipherBytes, cipherLength, decrypted, aes_key, 128, aes_iv);
